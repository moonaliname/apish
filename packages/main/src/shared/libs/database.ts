...
export const createDBTables = async () => {
  ...
  db.schema.hasTable('schema').then(function (exists) {
    if (!exists) {
  ...
  db.schema.hasTable('endpoint').then(function (exists) {
    if (!exists) {
      return db.schema.createTable('endpoint', (table) => {
        ...
        table.integer('schema_id').unsigned().references('schema.id')
        table.foreign('schema_id').references('schema.id').onDelete('SET NULL')
      })
    }
  })
  ...
  db.schema.hasTable('response').then(function (exists) {
    if (!exists) {
      return db.schema.createTable('response', (table) => {
        ...
        table.integer('schema_id').unsigned().references('schema.id')
        table.foreign('schema_id').references('schema.id').onDelete('CASCADE')
      })
    }
  })
  ...
  db.schema.hasTable('config').then(async function (exists) {
    if (!exists) {
      await db.schema.createTable('config', (table) => {
        ...
        table.integer('current_schema_id').unsigned().references('schema.id')
        table
